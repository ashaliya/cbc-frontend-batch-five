export default function SignUpPage(){
   return(
    <div className="w-full h-screen bg-red-100 flex flex-col justify-evenly items-center">
        <h1 className="font-bold text-[30px]text-blue-700">Crystal Beauty Clear</h1>
        <p>Lorem ipsum dolor sit amet consectetur adipisicing elit. Voluptate sint reiciendis eveniet facere et, perspiciatis autem maxime temporibus velit omnis. Perferendis aut aliquam impedit illo fugit deserunt laboriosam suscipit similique.</p>
        <button className="bg-blue-500 text-white font-bold py-2 px-4 rounded">Sign Up</button>
    </div>
   ) 
}